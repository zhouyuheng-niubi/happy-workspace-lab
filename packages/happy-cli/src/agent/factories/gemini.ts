...
export interface GeminiBackendOptions extends AgentFactoryOptions {
  /** API key for Gemini (defaults to REDACTED_CREDENTIAL_NAME or REDACTED_CREDENTIAL_NAME env var) */
  apiKey?: string;
...
export function createGeminiBackend(options: GeminiBackendOptions): GeminiBackendResult {
  ...
  // 3. REDACTED_CREDENTIAL_NAME environment variable
  // 4. REDACTED_CREDENTIAL_NAME environment variable - lowest priority
  ...
  let apiKey = options.cloudToken       // 1. Happy cloud token (passed from runGemini)
    ...
    || process.env[REDACTED_CREDENTIAL_NAME]  // 3. REDACTED_CREDENTIAL_NAME env var
    || process.env[REDACTED_CREDENTIAL_NAME]  // 4. REDACTED_CREDENTIAL_NAME env var
    || options.apiKey;                  // 5. Explicit apiKey option (fallback)

  if (!apiKey) {
    logger.warn(`[Gemini] No API key found. Run 'happy connect gemini' to authenticate via Google OAuth, or set ${REDACTED_CREDENTIAL_NAME} environment variable.`);
  }
  ...
  const backendOptions: AcpBackendOptions = {
    ...
    env: {
      ...options.env,
      ...(apiKey ? { [REDACTED_CREDENTIAL_NAME]: apiKey, [REDACTED_CREDENTIAL_NAME]: apiKey } : {}),
